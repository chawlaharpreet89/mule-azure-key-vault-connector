package com.pwc.mule.connector.akv.provider.api.client;

import com.pwc.mule.connector.akv.provider.api.client.model.*;
import com.pwc.mule.connector.akv.provider.api.error.CertificateNotFoundException;
import com.pwc.mule.connector.akv.provider.api.error.KeyNotFoundException;
import com.pwc.mule.connector.akv.provider.api.error.SecretNotFoundException;
...
public class AzureKeyVaultClient extends AzureClient {
  ...
  /**
   * Retrieves a secret list out of Azure Key Vault.
   *
   * @param maxResults    Number of secrets to be retrieved
   * @return SecretList       Object containing the secret and associated metadata
   */
  public SecretList getSecretList() throws DefaultMuleException {
	    HttpRequest request = getAuthenticatedHttpRequestBuilder()
	        .uri(getHttpBaseUri() + BASE_SECRET_PATH)
	        .addQueryParam(PARAM_API_VERSION, API_VERSION)
	        //.addQueryParam(MAXRESULTS, maxResults)
	    ...
	    try {
	      ...
	      LOGGER.error("Error retrieving secret list at ", e);
	      throw new DefaultMuleException("Error retrieving secret list at ", e);
	    }
	  }
